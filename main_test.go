...
func TestDo(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Should check the content generate by the interpolator is correct",
		...
		{name: "Should check the content generate by the interpolator is correct using varsContent",
			args: args{str: "I'm {{ .name | trim }} and I want to {{ .main_topic | upper  }} because I would like to see a film related with {{ .favorite_superhero | title }}",
				vars: VarsContent{
					"name":               "            Jose                 ",
					"main_topic":         "restore the snyderverse",
